...
class Renderer:
    ...
    def _get_raft_model(self):
        """Lazy load and cache RAFT model."""
        if not hasattr(self, '_raft_model'):
            device = self._device
            # Use raft_small for speed, raft_large for accuracy
            self._raft_model = raft_small(weights="DEFAULT", progress=False).to(device)
        ...
        **kwargs
        ...
        if is_drag:
            ...
            if self.feat_refs is None:
                ...
                # Store reference RGB image for optical flow (range [-1, 1])
                self.img0 = img.detach().clone()

            # # Point tracking with feature matching (original, kept for reference)
            # with torch.no_grad():
            ...
            # Point tracking with hybrid optical flow + feature matching
            with torch.no_grad():
                # Compute optical flow from previous reference image to current image
                img0_norm = (self.img0 + 1) / 2  # range [0,1]
                img_curr_norm = (img + 1) / 2
                raft_model = self._get_raft_model()
                list_of_flows = raft_model(img0_norm, img_curr_norm)
                flow = list_of_flows[-1]  # shape (1,2,H,W)

                # Sample flow at point locations
                points_tensor = torch.tensor(points, device=self._device, dtype=torch.float32)  # (N,2) each row (y, x)
                norm_points = torch.zeros_like(points_tensor)
                norm_points[:, 0] = (points_tensor[:, 0] / (h - 1)) * 2 - 1  # y (height)
                norm_points[:, 1] = (points_tensor[:, 1] / (w - 1)) * 2 - 1  # x (width)
                norm_points = norm_points.unsqueeze(0).unsqueeze(2)  # (1, N, 1, 2)
                sampled_flow = F.grid_sample(flow, norm_points, align_corners=True)  # (1,2,N,1)
                sampled_flow = sampled_flow.squeeze(3).squeeze(0).t()  # (N,2) where col0=dx, col1=dy
                dx = sampled_flow[:, 0].cpu().numpy()
                dy = sampled_flow[:, 1].cpu().numpy()

                # For each point, perform local feature matching around flow-predicted position
                for j in range(len(points)):
                    # Flow-predicted position
                    p_flow_y = points[j][0] + dy[j]
                    p_flow_x = points[j][1] + dx[j]
                    
                    # Local search window around flow prediction (radius r2)
                    r = round(r2 / 512 * h)
                    up = max(int(round(p_flow_y)) - r, 0)
                    down = min(int(round(p_flow_y)) + r + 1, h)
                    left = max(int(round(p_flow_x)) - r, 0)
                    right = min(int(round(p_flow_x)) + r + 1, w)
                    feat_patch = feat_resize[:, :, up:down, left:right]
                    L2 = torch.linalg.norm(feat_patch - self.feat_refs[j].reshape(1, -1, 1, 1), dim=1)
                    min_val, idx = torch.min(L2.view(1, -1), -1)
                    width = right - left
                    p_nn_y = float(idx.item() // width + up)
                    p_nn_x = float(idx.item() % width + left)
                    d_nn = min_val.item()
                    
                    # Fusion weight based on texture sparsity
                    texture_score = torch.std(feat_patch, dim=[2,3]).mean().item()
                    # Map texture score to weight using exponential decay
                    tau = 2.0  # tuning parameter
                    weight = 1 - math.exp(-texture_score / tau)
                    weight = max(0.0, min(1.0, weight))
                    print(f'Point {j}: texture_score={texture_score:.4f}, weight={weight:.4f}')

                    # Final position as weighted average
                    p_final_y = weight * p_nn_y + (1 - weight) * p_flow_y
                    p_final_x = weight * p_nn_x + (1 - weight) * p_flow_x
                    
                    # Clamp to image boundaries
                    p_final_y = max(0.0, min(h - 1, p_final_y))
                    p_final_x = max(0.0, min(w - 1, p_final_x))
                    
                    points[j] = [float(p_final_y), float(p_final_x)]
            ...
            # Update reference image for next iteration (continuous tracking)
            self.img0 = img.detach().clone()
