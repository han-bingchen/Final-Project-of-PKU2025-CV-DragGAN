...
def train_lora(image,
    ...
    if vae_path == "default":
        ...
        # Check if vae_path points to a model directory with vae subfolder
        vae_subfolder_path = os.path.join(vae_path, "vae")
        if os.path.exists(vae_subfolder_path) and os.path.isdir(vae_subfolder_path):
            # vae_path is a model directory, load from vae subfolder
            vae = AutoencoderKL.from_pretrained(vae_path, subfolder="vae", revision=None)
        else:
            # vae_path points directly to vae directory
